...
import { getSubjectsCode1List, getReportPElements, removeReportPElement, addReportPElement, updateReportPSubjects } from '../../api/index'
export default {
  data () {
    return {
      filters: {
      ...
      addFormRules: {
        ...
        type: [
          { type: 'string', required: true, message: '请选择类型', trigger: 'blur' }
        ]
      },
      // 新增界面数据
      addForm: {
  ...
  methods: {
    ...
    getElements () {
      ...
      getReportPElements(para).then((res) => {
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true
        let para = { id: row.id }
        removeReportPElement(para).then((res) => {
          this.listLoading = false
          this.$message({
            message: '删除成功',
            type: 'success'
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            addReportPElement(para).then((res) => {
    ...
    updateSubjects: function () {
      this.$confirm('确认提交吗？', '提示', {}).then(() => {
        ...
        updateReportPSubjects(para).then((res) => {
          this.addLoading = false
          this.$message({
            message: '提交成功',
            type: 'success'
